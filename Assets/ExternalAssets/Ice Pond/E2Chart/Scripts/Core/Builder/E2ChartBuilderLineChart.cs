    ...
    public class LineChartBuilder : E2ChartBuilder
    {
        ...
        void CreateItemsCategory()
        {
            ...
            for (int i = 0; i < dataInfo.seriesCount; ++i)
            {
                ...
                if (options.plotOptions.colorMode == E2ChartOptions.ColorMode.ByData) point.pointColors = colorList.ToArray();
            ...
            if (options.chartStyles.lineChart.enableLine)
            {
                ...
                for (int i = 0; i < dataInfo.seriesCount; ++i)
                {
                    ...
                    if (options.plotOptions.colorMode == E2ChartOptions.ColorMode.ByData) line.pointColors = colorList.ToArray();
            ...
            if (options.chartStyles.lineChart.enableShade)
            {
                ...
                for (int i = 0; i < dataInfo.seriesCount; ++i)
                {
                    ...
                    shade.color = c;
                    if (options.plotOptions.colorMode == E2ChartOptions.ColorMode.ByData)
                    {
                        if (options.plotOptions.seriesColors != null && options.plotOptions.seriesColors.Length > 0)
                        {
                            shade.pointColors = colorList.ToArray();
                            for (int n = 0; n < shade.pointColors.Length; ++n)
                            {
                                Color cc = shade.pointColors[n]; cc.a = options.chartStyles.lineChart.shadeOpacity;
                                shade.pointColors[n] = cc;
                            }
                        }
                    }
        ...
        protected override void CreateCustomLabels()
        {
            if (customLabels.labelInfo == null) return;
            if (labelRect == null)
            {
                labelRect = E2ChartBuilderUtility.CreateEmptyRect("Labels", contentRect, true);
                labelRect.offsetMin = dataRect.offsetMin;
            ...
            float labelRotation = E2ChartBuilderUtility.GetLabelRotation(options.label.rotationMode);
            for (int i = 0; i < customLabels.labelInfo.Length; ++i)
            {
                E2ChartCustomLabels.CustomLabelInfo info = customLabels.labelInfo[i];
                if (info.seriesIndex >= dataInfo.activeDataCount.Length || dataInfo.activeDataCount[info.seriesIndex] == 0) continue;
                if (info.dataIndex >= dataInfo.dataShow[info.seriesIndex].Length || !dataInfo.dataShow[info.seriesIndex][info.dataIndex]) continue;
                Vector2 anchorLabel = GetItemAnchorPosition(info.seriesIndex, info.dataIndex, info.labelAnchoredPosition);
                Vector2 anchorIcon = GetItemAnchorPosition(info.seriesIndex, info.dataIndex, info.iconAnchoredPosition);

                if (!string.IsNullOrEmpty(info.labelContent) && IsAnchorPointInsideRect(anchorLabel))
                {
                    float offsetLabel = info.labelOffset * Mathf.Sign(pointList[info.seriesIndex].dataValue[info.dataIndex]);
                    E2ChartText label = E2ChartBuilderUtility.CreateText("Label", labelRect, info.labelTextOption, options.plotOptions.generalFont);
                    label.text = dataInfo.GetLabelText(info.labelContent, options.label.numericFormat, info.seriesIndex, info.dataIndex);
                    label.rectTransform.localRotation = Quaternion.Euler(0.0f, 0.0f, labelRotation);
                    label.rectTransform.anchorMin = label.rectTransform.anchorMax = anchorLabel;
                    label.rectTransform.anchoredPosition = options.rectOptions.inverted ? new Vector2(offsetLabel, 0.0f) : new Vector2(0.0f, offsetLabel);
                    label.rectTransform.sizeDelta = Vector2.zero;
                }

                if (info.iconSprite != null && IsAnchorPointInsideRect(anchorIcon))
                {
                    float offsetIcon = info.iconOffset * Mathf.Sign(pointList[info.seriesIndex].dataValue[info.dataIndex]);
                    Image icon = E2ChartBuilderUtility.CreateImage("Icon", labelRect);
                    icon.color = info.iconColor;
                    icon.sprite = info.iconSprite;
                    icon.rectTransform.anchorMin = icon.rectTransform.anchorMax = anchorIcon;
                    icon.rectTransform.anchoredPosition = options.rectOptions.inverted ? new Vector2(offsetIcon, 0.0f) : new Vector2(0.0f, offsetIcon);
                    icon.rectTransform.sizeDelta = info.iconSize;
                }
            }
        }
        ...
        Vector2 GetItemAnchorPosition(int seriesIndex, int dataIndex, float ratio = 1.0f)
        {
            ...
            {
                E2ChartGraphicLineChartPointLinear point = (E2ChartGraphicLineChartPointLinear)pointList[seriesIndex];
                float posX = point.dataPos[dataIndex];
